...
command -v zenity &>/dev/null || { echo "Zenity no instalado"; exit 1; }
...
OUTPUT=$(zenity --file-selection --directory --title="Seleccionar salida")
[[ -z "$OUTPUT" ]] && exit 0

if [[ "$ACTION" == "Desempaquetar" ]]; then
    FILES=$(zenity --file-selection --multiple --separator="|" --file-filter="*.jar")
else
...
) | zenity --progress --auto-close --title="Procesando..."
